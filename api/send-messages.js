...
export default async function handler(req, res) {
  if (req.method === "POST") {
    ...
    const filteredMessages = mapMessagesWithFirebaseId(filteredPackages, messages);

    var updatedPackages = {};

    try {
      filteredMessages.forEach(async (pckg) => {
    ...
    res.status(200).json({ res: `packages updated successfully with ${Object.keys(updatedPackages).length}` });
  } else {
